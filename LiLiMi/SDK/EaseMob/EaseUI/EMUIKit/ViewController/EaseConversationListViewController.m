...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section==0) {
        ...
        //设置聊天头像
        [[EaseConversationCell appearance] setAvatarSize:40.f];
        [[EaseConversationCell appearance] setAvatarCornerRadius:20.f];
        // Configure the cell...
        if (cell == nil) {
            cell = [[EaseConversationCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
        }
        
        if ([self.systemdataArray count] <= indexPath.row) {
            return cell;
        }
        
        NotiListModel*model = [self.systemdataArray objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section==0) {
        if (_delegate && [_delegate respondsToSelector:@selector(notiListViewController:didSelectConversationModel:)]) {
            NotiListModel *model = [self.systemdataArray objectAtIndex:indexPath.row];
            [_delegate notiListViewController:self didSelectConversationModel:model];
        }
    }
    else
    {
...
- (void)tableViewDidTriggerHeaderRefresh
{
    ...
    dispatch_async(refreshQueue, ^{
        ...
        [weakself.systemdataArray removeAllObjects];
        for (int i=0; i<2; i++) {
            NotiListModel *model = [[NotiListModel alloc]init];
            model.title=(i==0)?@"系统消息":@"互动消息";
            model.detail=(i==0)?@"粒粒米提醒您：还有个人资料未完善":@"Halo009回复了你：你是想和谁一起玩的";
            model.timestamp=@"5分钟前";
            if (model) {
                [weakself.systemdataArray addObject:model];
            }
        }
